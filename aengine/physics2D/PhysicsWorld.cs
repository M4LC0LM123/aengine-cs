...
public sealed class PhysicsWorld {
    ...
    private void resolveCollision(RigidBody2D bodyA, RigidBody2D bodyB, Vector2 normal, float depth) {
        ...
        bodyA.setLinearVelocity(bodyA.getLinearVelocity() - j / bodyA.mass * normal);
        bodyB.setLinearVelocity(bodyB.getLinearVelocity() + j / bodyB.mass * normal);
    }
    ...
    private bool collide(RigidBody2D bodyA, RigidBody2D bodyB, out Vector2 normal, out float depth) {
        ...
        if (physicsShapeA is PhysicsShape.BOX) {
            if (physicsShapeB is PhysicsShape.BOX) {
                return Collisions.checkPolyOverlap(bodyA.getTransformedVertices(), bodyB.getTransformedVertices(),
                    out normal, out depth);
            } else if (physicsShapeB is PhysicsShape.CIRCLE) {
                bool result = Collisions.checkPolyCircleOverlap(bodyB.getPosition(), bodyB.radius,
            ...
            if (physicsShapeB is PhysicsShape.BOX) {
                return Collisions.checkPolyCircleOverlap(bodyA.getPosition(), bodyA.radius,
                ...
                return Collisions.checkCircleOverlap(bodyA.getPosition(), bodyA.radius, bodyB.getPosition(),
                    bodyA.radius, out normal, out depth);
            }
        }
