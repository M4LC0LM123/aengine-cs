        ...
        while (!WindowShouldClose()) {
            ...
            for (int i = 0; i < bodies.Count - 1; i++) {
                ...
                for (int j = i + 1; j < bodies.Count; j++) {
                    ...
                    if (bodyA.shape is PhysicsShape.BOX && bodyB.shape is PhysicsShape.CIRCLE) {
                        if (Collisions.checkPolyCircleOverlap(bodyB.getPosition(), bodyB.radius, bodyA.getTransformedVertices(), out Vector2 normal, out float depth)) {
                            bodyA.move(normal * depth / 2 * 25);
                            bodyB.move(-normal * depth / 2 * 25);
                        }
                    } else if (bodyB.shape is PhysicsShape.BOX && bodyA.shape is PhysicsShape.CIRCLE) {
                        if (Collisions.checkPolyCircleOverlap(bodyA.getPosition(), bodyA.radius, bodyB.getTransformedVertices(), out Vector2 normal, out float depth)) {
                            bodyA.move(-normal * depth / 2 * 25);
                            bodyB.move(normal * depth / 2 * 25);
                        }
                    }
                    
                    // if (Collisions.checkPolyOverlap(bodyA.getTransformedVertices(), bodyB.getTransformedVertices(), out Vector2 normal, out float depth)) {
                    //     bodyA.move(-normal * depth / 2 * 25);
                    //     bodyB.move(normal * depth / 2 * 25);
                    // }
